from cluster_original import Cluster
from clustering import Clustering
from superinstance_kshape import SuperInstance
...
class COBRAS:
    ...
    def determine_starting_k(self):
        self.clustering = Clustering([Cluster([SuperInstance(self.A, list(range(len(self.labels))), self.train_indices, None)])])
        ...
        while not must_link_found:
            print("entering the while in determine_starting_k")
            split_labels = self.split_superinstance(superinstance)
            print("these are the split labels")
            print(split_labels)
    ...
    def determine_starting_k_for_superinstance(self, superinstance):
        ...
        while not must_link_found:
            ...
            data_to_cluster = self.A[si.indices,:]



            if len(si.indices) == 2:
                split_labels = [0,1]
                clusters = [(self.A[si.indices[0],:],[0]),(self.A[si.indices[1],:],[1])]
                ...
                clusters = kshape(data_to_cluster,2)





            si_containing_training = self.merge_superinstances_without_training(si, clusters)
    ...
    def merge_superinstances_without_training(self, to_split, split_labels):
        ...
        for new_si_centroid, new_si_idx in split_labels:

        #for new_si_idx in range(len(set(split_labels))):
            # go from super instance indices to global ones


            cur_indices = [to_split.indices[idx] for idx in new_si_idx]
        ...
        for indices, centroid in no_training:
            ...
            for training_si in training:
                cur_dist,_ = _sbd(training_si.prototype,centroid)
    ...
    def add_new_clusters_from_split(self, to_split, split_labels):
        # split_labels is now a list of (centroid,indices) pairs
    ...
    def cluster(self):
        ...
        print("started determining starting k")
        depth = self.determine_starting_k()
        print("finished determining starting k")
        ...
        pred = kshape(self.A,initial_k)


        superinstances = self.merge_superinstances_without_training(
            SuperInstance(self.A, list(range(len(self.labels))), self.train_indices, None), pred)
    ...
    def does_split_contain_train_points(self, superinstance, split):
        for clust_id in range(2):
            clust_indices = [superinstance.indices[idx] for idx, c in enumerate(split[clust_id][1]) if superinstance.indices[idx] in self.train_indices]
    ...
    def split_superinstance(self, superinstance, split_level=2):
        ...
        if tuple(set(superinstance.indices)) in self.split_cache:
            return self.split_cache[tuple(set(superinstance.indices))]
        else:
            if len(superinstance.indices) <= split_level:
                self.split_cache[tuple(set(superinstance.indices))] = list(range(len(superinstance.indices)))
            else:
                #data_to_cluster = self.A[np.ix_(superinstance.indices, superinstance.indices)]
                data_to_cluster = self.A[superinstance.indices,:]
                #km = SpectralClustering(split_level,affinity="precomputed")
                #km.fit(data_to_cluster)
                clusters = kshape(data_to_cluster,split_level)
                self.split_cache[tuple(set(superinstance.indices))] = clusters
            return self.split_cache[tuple(set(superinstance.indices))]
